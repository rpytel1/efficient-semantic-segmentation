...
class OwnCityscapes(Cityscapes):
    ...
    def __init__(self, root, split='train', transform=None, target_transform=None, transforms=None):
        ...
        for file_dir in os.listdir(self.images_dir):
            for file_name in os.listdir(os.path.join(self.images_dir,file_dir)):
                self.images.append(os.path.join(self.images_dir, file_dir, file_name))
                if split != 'test':
                    target_name = '{}_{}'.format(file_name.split('_leftImg8bit')[0],
                                                     'gtFine_labelIds.png')
                    self.targets.append(os.path.join(self.targets_dir, file_dir, target_name))
